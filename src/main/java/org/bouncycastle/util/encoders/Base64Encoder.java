...
public class Base64Encoder implements Encoder {
   ...
   public int decode(String data, OutputStream out) throws IOException {
      ...
      for(int ix = this.nextI((String)data, i, finish); ix < finish; ix = this.nextI(data, ix, finish)) {
         byte b1 = this.decodingTable[data.charAt(ix++)];
         ix = this.nextI(data, ix, finish);
         byte b2 = this.decodingTable[data.charAt(ix++)];
         ix = this.nextI(data, ix, finish);
         byte b3 = this.decodingTable[data.charAt(ix++)];
         ix = this.nextI(data, ix, finish);
         byte b4 = this.decodingTable[data.charAt(ix++)];
         out.write(b1 << 2 | b2 >> 4);
         out.write(b2 << 4 | b3 >> 2);
         out.write(b3 << 6 | b4);
         length += 3;
